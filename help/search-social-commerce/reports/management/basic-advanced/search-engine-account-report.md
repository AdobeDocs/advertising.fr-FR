---
title: '[!UICONTROL Search Engine Account Report]'
description: En savoir plus sur le [!UICONTROL Search Engine Account Report].
exl-id: 83f3c42e-8488-48e5-8b47-199b52deb2dd
feature: Search Reports, Search Basic Reports
TQID: https://experienceleague.adobe.com/V7IzT6h-3c3fA0yiPip6JTxo5nHAYLouIiCjmA0Zahg
product_v2:
  - id: a829a185-511f-4bf8-8dcf-9e684f8011cf
role_v2:
  - id: b69b2659-1057-424e-8fc5-ed9e016dc554
topic_v2:
  - id: e1e0219c-f879-479f-8427-888ed2a6e9c2
source-git-commit: 527ca2bb74de388c13ba1ce5bde3f8be1cead8d0
workflow-type: tm+mt
source-wordcount: 98
ht-degree: 0%

---

# [!UICONTROL Search Engine Account Report]

La [!UICONTROL Search Engine Account Report] inclut les données de coût, de clic et (éventuellement) de conversion pour un ou plusieurs comptes de réseau publicitaire avec des campagnes. Par défaut, les données incluent une ligne pour chaque compte applicable pour chaque unité de temps dans la période spécifiée, et les lignes sont dans l’ordre croissant d’abord par réseau publicitaire, puis par compte.

Vous pouvez afficher les données des 36 mois précédents.

## Colonnes par défaut

Pour la description de toutes les colonnes par défaut et personnalisées, voir « [Colonnes de rapport pour les rapports de base et avancés](basic-advanced-report-columns.md) ».

* [!UICONTROL Account]
* [!UICONTROL Search Engine]
* [!UICONTROL Start Date]
* [!UICONTROL End Date]
* [!UICONTROL Impressions]
* [!UICONTROL Cost]
* [!UICONTROL Clicks]
* [!UICONTROL CPC]
* [!UICONTROL Avg Position]
* [!UICONTROL Impr. (Abs. Top) %]
* [!UICONTROL Impr. (Top) %]
* [!UICONTROL ViewThroughConversions]

>[!MORELIKETHIS]
>
>* [À propos des rapports de base et avancés](basic-advanced-report-about.md)
>* [Générer un rapport de base ou avancé](basic-advanced-report-generate.md)
>* [Paramètres de rapport de base et avancés](basic-advanced-report-settings.md)
