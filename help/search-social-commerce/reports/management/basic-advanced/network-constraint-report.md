---
title: '[!UICONTROL Network Constraint Report]'
description: En savoir plus sur le [!UICONTROL Network Constraint Report].
exl-id: ab908d86-2d37-476c-b2ac-f74c2f3a9a6d
feature: Search Reports, Search Basic Reports
TQID: https://experienceleague.adobe.com/AluSzqEjEoZxnl4Qx7MG2wk7A3NsnsDAKpJ2kU6z0Bk
product_v2: id: a829a185-511f-4bf8-8dcf-9e684f8011cf
role_v2: id: b69b2659-1057-424e-8fc5-ed9e016dc554
topic_v2: id: e1e0219c-f879-479f-8427-888ed2a6e9c2
source-git-commit: 527ca2bb74de388c13ba1ce5bde3f8be1cead8d0
workflow-type: tm+mt
source-wordcount: 111
ht-degree: 0%

---

# [!UICONTROL Network Constraint Report]

La [!UICONTROL Network Constraint Report] affiche tous les budgets de réseau publicitaire minimum et maximum spécifiés ainsi que les dépenses réelles (coût). Le rapport inclut des données de coût, de clic, d’impression, d’affichage publicitaire et (éventuellement) de conversion pour un ou plusieurs portefeuilles. Par défaut, les données incluent une ligne pour chaque compte de réseau publicitaire applicable dans la période spécifiée. Les lignes sont dans l’ordre croissant, d’abord par portfolio, puis par réseau publicitaire.

Les données sont disponibles à partir du 15 octobre 2022.<!-- [Later: You can view data for the previous NN days.] -->

## Colonnes par défaut

Pour la description de toutes les colonnes par défaut et personnalisées, voir « [Colonnes de rapport pour les rapports de base et avancés](basic-advanced-report-columns.md) ».

* [!UICONTROL Portfolio]
* [!UICONTROL Portfolio Group Name]
* [!UICONTROL Account]
* [!UICONTROL Network]
* [!UICONTROL Start Date]
* [!UICONTROL End Date]
* [!UICONTROL Impressions]
* [!UICONTROL Cost]
* [!UICONTROL Clicks]
* [!UICONTROL CPC]
* [!UICONTROL Avg Position]
* [!UICONTROL ViewThroughConversions]
* [!UICONTROL Min Spend (%)]
* [!UICONTROL Max Spend (%)]

>[!MORELIKETHIS]
>
>* [À propos des rapports de base et avancés](basic-advanced-report-about.md)
>* [Générer un rapport de base ou avancé](basic-advanced-report-generate.md)
>* [Paramètres de rapport de base et avancés](basic-advanced-report-settings.md)
