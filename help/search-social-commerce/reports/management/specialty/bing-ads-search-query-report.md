...
description: En savoir plus sur le [!UICONTROL Bing Ads Search Query Report].
...
La [!UICONTROL Bing Ads Search Query Report] inclut des données de coût, de clic, d’impression et (éventuellement) des mesures dérivées pour les termes de requête des utilisateurs pour un ou plusieurs groupes publicitaires. Par défaut, les données incluent une ligne pour chaque terme de recherche qui a généré des impressions ou des clics pour chaque groupe d’annonces applicable pour chaque unité de temps dans la période spécifiée. Les lignes sont classées par ordre croissant par nom de groupe publicitaire, puis par terme de recherche par défaut. Le rapport n’inclut pas le mot-clé auquel le terme de recherche a été associé, ni les données de conversion suivies par [!DNL Adobe].
