...
# (Nouvelle interface utilisateur) Gérer les rapports de vue de données à partir de la vue [!UICONTROL Ads]

*Fonction*

Vous pouvez générer un rapport qui inclut les lignes de données d’une ou de plusieurs publicités dans la vue [!UICONTROL Ads], puis télécharger le rapport sous la forme d’un fichier de feuille de calcul Excel Microsoft (format XLXS). Le rapport inclut toutes les colonnes visibles dans la vue.
