...
+++Je souhaite arrêter d’utiliser le service de suivi de conversion Adobe Advertising pour une ou plusieurs campagnes ou comptes. Comment puis-je supprimer rapidement le code de suivi des URL de suivi ?
...
+++Comment savoir quelle propriété de transaction provient d’un flux de données ou est suivie par la balise de suivi de conversion Adobe Advertising ?

Dans un [!UICONTROL Transaction Report], vous pouvez déterminer si une propriété de transaction incluse a été suivie par le pixel de suivi de conversion Adobe Advertising si vous incluez la colonne personnalisée &quot;[!UICONTROL Tracking URL].&quot; Le suivi des URL avec le pixel de suivi Advertising Adobe commence par `http://pixel.everesttech.net`.
+++

+++Que sont les transactions orphelines ?

Les transactions orphelines sont des événements de transaction qui ne peuvent pas être associés à un mot-clé ou à une publicité spécifique. Adobe Advertising attribue la transaction/le chiffre d’affaires à un mot-clé ou à une publicité en faisant correspondre les ID de suivi reçus avec l’événement de chiffre d’affaires à l’identifiant de suivi unique dans l’URL de suivi du mot-clé ou de la publicité.
