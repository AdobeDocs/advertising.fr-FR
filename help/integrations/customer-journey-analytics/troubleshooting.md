...
| Aucune requête d’interaction ou de collecte n’est envoyée à edge.adobedc.net. | Voir « [Problèmes d’installation et de configuration](#issues-installation-setup) » > « [L’extension WebSDK ne s’initialise pas](#websdk-extension-doesn't-initialize) » |
...
* Aucune requête d’interaction ou de collecte n’est envoyée à edge.adobedc.net.

#### Causes possibles et vérification/résolution

| Cause | Correctif |
| ----- | --- |
| Bibliothèque non publiée ou à l’état de brouillon | Accédez à [Flux de publication](https://experienceleague.adobe.com/fr/docs/experience-platform/tags/publish/publishing-flow) et assurez-vous que la bibliothèque contenant l’extension WebSDK est à l’état approuvé/publié. |
| Environnement manquant ou incorrect du code incorporé | Vérifiez que le [[!DNL Tags] code intégré](https://experienceleague.adobe.com/fr/docs/experience-platform/tags/publish/environments/environments) sur la page web référence l’environnement approprié (Dev/Stage/Prod). Recherchez l’environnement dans la balise `<head>` pour la balise de script `//assets.adobedtm.com/...`. |
| Conflit de charge asynchrone et synchrone | Assurez-vous qu’un seul [[!DNL Tags] code intégré](https://experienceleague.adobe.com/fr/docs/experience-platform/tags/publish/environments/environments) est présent par page web. Les codes incorporés en double entraînent des conditions de concurrence. |
| Blocage de la politique de sécurité du contenu (CSP) | Ajoutez `edge.adobedc.net` `and assets.adobedtm.com` à vos [`connect-src` CSP et `script-src` directives](https://experienceleague.adobe.com/fr/docs/experience-platform/collection/use-cases/configuring-a-csp). |
...
* Aucune donnée n’apparaît dans les rapports Adobe Analytics ou Adobe Advertising.
...
| Les services de flux de données ne sont pas activés pour la propriété de balise. | [Ouvrez les paramètres du flux de données](https://experienceleague.adobe.com/fr/docs/experience-platform/datastreams/configure) et assurez-vous que les services suivants sont activés :<ul><li>Adobe Advertising (pour la conversion/synchronisation de l’audience)</li><li>Adobe Experience Platform (pour l’ingestion de profils)</li></ul> |
| Incompatibilité de sandbox | Assurez-vous que le flux de données appartient au même sandbox [&#128279;](https://experienceleague.adobe.com/fr/docs/experience-platform/sandbox/home) que votre schéma et votre jeu de données. Une erreur courante est de créer un flux de données dans le sandbox de production, mais de pointer les schémas vers le sandbox de développement. |
...
| Aucune zone d’événements de visionnage publicitaire n’est disponible dans Experience Platform après les déclenchements de l’appel d’interaction de visionnage publicitaire . | Vérifiez qu’un annonceur est configuré et activé dans la section [[!UICONTROL Advertiser] de la configuration de l’extension WebSDK](https://experienceleague.adobe.com/fr/docs/experience-platform/tags/extensions/client/web-sdk/configure/advertising). Si l’annonceur a été saisi manuellement, sélectionnez à nouveau l’annonceur dans la liste déroulante [!UICONTROL Advertiser] . Une fois que vous avez configuré l’annonceur, recréez et republiez la bibliothèque. |
...
| Cause | Correctif |
| ----- | --- |
| Les cookies tiers sont bloqués. | Migrez vers la collecte de données CNAME propriétaire en [configurant un identifiant de cookie propriétaire dans la configuration Edge Network du flux de données](https://experienceleague.adobe.com/fr/docs/experience-platform/datastreams/configure). |
| `idMigrationEnabled` est défini sur `false` tant qu’un cookie de `s_ecid` hérité est présent. | [Définissez `idMigrationEnabled: true` dans la configuration de base du SDK Web](https://experienceleague.adobe.com/fr/docs/experience-platform/tags/extensions/client/web-sdk/configure/identity) pour migrer l’ECID existant à partir des cookies `s_ecid` ou `AMCV_`. |
...
* La règle est enregistrée et incluse dans la [version de bibliothèque](https://experienceleague.adobe.com/fr/docs/experience-platform/tags/publish/builds) active.
...
* Les conditions de la règle ne sont pas trop restrictives. Testez en [supprimant temporairement les conditions](https://experienceleague.adobe.com/fr/docs/experience-platform/tags/ui/rules) pour isoler le problème.
* L’ordre des règles est correct. Si plusieurs règles partagent le même événement, vérifiez l’[ordre des règles](https://experienceleague.adobe.com/fr/docs/experience-platform/tags/ui/rules).
...
| Cause | Correctif |
| ----- | --- |
| Les modifications n’ont pas été ajoutées à une bibliothèque. | Dans le workflow de publication, vérifiez que vos modifications ont été ajoutées à une [bibliothèque](https://experienceleague.adobe.com/fr/docs/experience-platform/tags/publish/libraries) dans l’environnement de développement. Si vous [apportez des modifications à la bibliothèque](https://experienceleague.adobe.com/fr/docs/experience-platform/tags/publish/libraries#manage-library-changes), ajoutez les ressources, puis enregistrez et créez la bibliothèque. |
...
| Le code incorporé est destiné à un environnement incorrect. | Vérifiez que le code incorporé sur la page correspond à l’environnement approprié. Par exemple, si vous testez le comportement en production, vérifiez que le code incorporé est bien celui de la production. |
| Échec silencieux de la création de la bibliothèque. | Dans le workflow de publication, vérifiez si la bibliothèque affiche un état [!UICONTROL Build Failed]. Ouvrez la bibliothèque et consultez le journal de génération. Les causes courantes des échecs de génération sont des configurations de règle non valides ou des conflits de version d’extension. |
