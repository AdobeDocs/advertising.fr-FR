...
| Environnement manquant ou incorrect du code incorporé | Vérifiez que le [[!DNL Tags] code intégré](https://experienceleague.adobe.com/en/docs/experience-platform/tags/publish/environments/environments) sur la page web référence l’environnement approprié (Dev/Stage/Prod). Recherchez l’environnement dans la balise `<head>` pour la balise de script `//assets.adobedtm.com/...`. |
| Conflit de charge asynchrone et synchrone | Assurez-vous qu’un seul [[!DNL Tags] code intégré](https://experienceleague.adobe.com/en/docs/experience-platform/tags/publish/environments/environments) est présent par page web. Les codes incorporés en double entraînent des conditions de concurrence. |
| Blocage de la politique de sécurité du contenu (CSP) | Ajoutez `edge.adobedc.net` `and assets.adobedtm.com` à vos [`connect-src` CSP et `script-src` directives](https://experienceleague.adobe.com/en/docs/experience-platform/collection/use-cases/configuring-a-csp). |
...
| Cause | Correctif |
| ----- | --- |
| Les cookies tiers sont bloqués. | Migrez vers la collecte de données CNAME propriétaire en [configurant un identifiant de cookie propriétaire dans la configuration Edge Network du flux de données](https://experienceleague.adobe.com/en/docs/experience-platform/datastreams/configure). |
...
* La règle est enregistrée et incluse dans la [version de bibliothèque](https://experienceleague.adobe.com/en/docs/experience-platform/tags/publish/builds) active.
...
* Les conditions de la règle ne sont pas trop restrictives. Testez en [supprimant temporairement les conditions](https://experienceleague.adobe.com/en/docs/experience-platform/tags/ui/rules) pour isoler le problème.
* L’ordre des règles est correct. Si plusieurs règles partagent le même événement, vérifiez l’[ordre des règles](https://experienceleague.adobe.com/en/docs/experience-platform/tags/ui/rules).
