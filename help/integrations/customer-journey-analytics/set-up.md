...
### Collecter et envoyer des données d’Adobe Advertising vers Experience Platform Edge Network en tant que jeu de données {#dataset-datastream}
...
### Envoyez les données du site web de votre organisation au flux de données <!-- ?? --> {#tags-websdk}
...
1. Demandez à l’équipe chargée de votre compte Adobe de valider le suivi sur votre site web.

   ![Exemple de validation de la payload de suivi des clics publicitaires](/help/integrations/assets/cja-example-click-through-validation.png "Exemple de validation de la payload de suivi des clics publicitaires")
...
1. Dans Customer Journey Analytics, [créez ou modifiez une connexion](https://experienceleague.adobe.com/en/docs/analytics-platform/using/cja-connections/create-connection) qui inclut vos jeux de données et votre schéma Experience Platform.
   ...
   * Vérifiez que le bon sandbox (configuré par votre organisation) est sélectionné.
...
1. Vérifiez que les données s’affichent.

>[!TIP]
>
>Les événements récapitulatifs ajoutent généralement une petite quantité de données supplémentaires aux rapports, telles que quelques événements supplémentaires, une session supplémentaire par jour ou une personne supplémentaire par rapport. Ces ajouts sont négligeables par rapport aux événements web standard. Cependant, vous pouvez filtrer ces données d’événement de résumé supplémentaires en excluant les données du `00000000-0000-0000-0000-000000000000` ID de personne factice.Exemple d’exclusion de données à l’aide d’un ID de personne](/help/integrations/assets/cja-report-with-person-id.png "Exemple d’exclusion de données à l’aide d’un ID de personne")
