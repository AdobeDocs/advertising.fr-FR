...
Pour plus d’informations sur [!DNL Audience Analytics], y compris sur ses conditions préalables et son workflow, consultez la section « [Présentation d’Audience Analytics](https://experienceleague.adobe.com/docs/analytics/integration/audience-analytics/mc-audiences-aam.html?lang=fr) ».
...
* Connectez les [!DNL Analytics for Advertising]<!-- which doesn't include the last exposure event --> de données [!DNL Audience Analytics] et <!-- (which includes the user's last exposure event) --> pour déterminer un parcours holistique au site.
