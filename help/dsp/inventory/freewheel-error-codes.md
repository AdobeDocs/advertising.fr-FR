...
Les messages d’erreur pour les envois d’annonces ayant échoué peuvent provenir d’Advertising DSP ou de [!DNL FreeWheel]. Les messages d’erreur s’affichent dans la colonne [!UICONTROL API Response] de la boîte de dialogue de [[!UICONTROL Freewheel Status]](freewheel-check-status.md).
