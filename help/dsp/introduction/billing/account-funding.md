---
title: Financement du compte
description: Découvrez le financement des comptes pour DSP.
feature: DSP Introduction
exl-id: 95e1fd75-ed38-41e3-a464-afe5e23c1c22
source-git-commit: 9b5c00cdb0942ec4e7fbf69d7ce638ab50439915
workflow-type: tm+mt
source-wordcount: '263'
ht-degree: 0%

---

# Financement du compte Advertising DSP

DSP finance les comptes clients pour l’achat de médias et de services de l’une des deux façons suivantes :

* Les clients soumettent un montant prépayé, à partir duquel les fonds sont utilisés pour les services et les médias jusqu&#39;à ce qu&#39;un solde nul soit atteint.
* Adobe fournit une marge de crédit évaluée.

Tous les clients sont évalués et crédités lors de l’exécution du contrat par l’équipe du crédit Adobe. DSP finance par défaut les comptes clients au niveau de crédit affecté, sauf si un prépaiement a été versé. Les comptes sont financés dans les 24 heures suivant leur création.

À mesure que les clients dépensent en contrepartie de leur crédit ou de leur paiement anticipé, les fonds pour les médias et les services diminuent en fonction des dépenses jusqu&#39;à ce qu&#39;un solde nul soit atteint. Le crédit est ajusté au fur et à mesure du paiement des factures. Toutes les factures Adobe en suspens peuvent avoir une incidence sur votre financement DSP. Vous êtes responsable du paiement en temps voulu de toutes les factures Adobe. Si vous avez besoin d’un financement supérieur à votre solde utilisable, contactez l’équipe chargée de votre compte Adobe.

Le financement des comptes clients est mis à jour en temps quasi réel en [!DNL DSP]. Pour afficher votre solde utilisable, qui s&#39;affiche sous la forme « [!UICONTROL Usable Funds] » dans la devise d&#39;achat de votre compte, accédez à [!UICONTROL Settings] > [!UICONTROL Account]. Seuls les utilisateurs de DSP disposant des rôles utilisateur [!UICONTROL General] et [!UICONTROL Admin] peuvent voir le [!UICONTROL Usable Funds].

![Fonds utilisables pour un compte](/help/dsp/assets/account-usable-funds.png)

Adobe se réserve le droit de bloquer les dépenses de votre compte sur DSP pour des raisons qui incluent une violation de la politique de DSP [conditions relatives aux publicités](/help/policies/ad-requirements-policy.md) ou des conditions de confidentialité. Si Adobe bloque votre compte, les paramètres de votre compte incluent un avis de blocage. Contactez l’équipe chargée de votre compte Adobe pour déterminer la raison du blocage et la manière de corriger l’erreur.

>[!MORELIKETHIS]
>
>* [Devises prises en charge ](/help/dsp/currency.md)
